...
// Buddy - 8-bit pitbull puppy companion
...
function drawBuddy(ctx: CanvasRenderingContext2D, pup: PupState, theme: string) {
  const c = ctx;
...
function drawSignCanvas(ctx: CanvasRenderingContext2D, theme: string) {
  const c = ctx;
  const p = pupPal(theme);
  c.clearRect(0, 0, 18, 22);
  c.fillStyle = p.body;
  c.fillRect(8, 0, 2, 22);
  c.fillRect(0, 3, 18, 11);
  c.fillStyle = p.white;
  // B
  px(c, 2, 5); px(c, 2, 6); px(c, 2, 7); px(c, 2, 8); px(c, 2, 9); px(c, 3, 5); px(c, 3, 7); px(c, 3, 9); px(c, 4, 6); px(c, 4, 8);
  // U
  px(c, 6, 5); px(c, 6, 6); px(c, 6, 7); px(c, 6, 8); px(c, 7, 9); px(c, 8, 5); px(c, 8, 6); px(c, 8, 7); px(c, 8, 8);
  // D
  px(c, 10, 5); px(c, 10, 6); px(c, 10, 7); px(c, 10, 8); px(c, 10, 9); px(c, 11, 5); px(c, 11, 9); px(c, 12, 6); px(c, 12, 7); px(c, 12, 8);
  // Y
  px(c, 14, 5); px(c, 14, 6); px(c, 15, 7); px(c, 15, 8); px(c, 15, 9); px(c, 16, 5); px(c, 16, 6);
}
...
export default function Buddy({ theme }: { theme: string }) {
  ...
  useEffect(() => {
    // Draw doghouse and sign
    if (dhCanvasRef.current) drawDoghouseCanvas(dhCanvasRef.current.getContext('2d')!, theme);
    if (signCanvasRef.current) drawSignCanvas(signCanvasRef.current.getContext('2d')!, theme);
  }, [theme]);
  ...
  return (
    <div ref={containerRef}>
      ...
      <div
        ...
        className="flex items-end justify-center gap-0 cursor-pointer mb-4 pt-2"
      ...
      >
        ...
        <div className="flex flex-col items-center ml-1 mb-1">
          <canvas ref={signCanvasRef} width={18} height={22} style={{ width: 36, height: 44, imageRendering: 'pixelated' }} />
        </div>
      </div>
